...
\tableofcontents
 ...
 \section{${}\bmod p$ のアルゴリズム}
  ...
  \label{subsec:group-preliminaries}
   ...
   乗算と、乗算の逆元と、乗算の単位元が定義された集合のことを\defineRuby{群}{ぐん}と呼びます。
...
\end{itembox}
 ...
 \label{sec:mod-p-2}
  ...
  そのため、$\legendre{c + r}{p}$ か $\legendre{c - r}{p}$ のどちらかは1でどちらかは$-1$となります。定理\ref{thm:quadratic-residue-fermat}より、$x^{\frac{p-1}{2}}-1$ は全ての平方剰余$a$に対する$x-a$の積です。よって、$(x - c -r) (x - c + r)$ と $x^{\frac{p-1}{2}}-1$ の最大公約数をとると、$c\pm r$ のうち平方剰余の方だけが残ります。
 ...
 \label{sec:technique-using-polynomials}
  ...
  実装には double 型の演算を用いた FFT を用いました。FFT の部分はソースコード\ref{fft.cpp} と同じです。25行目において、1の$n$乗根$\zeta$の$i$乗を愚直に計算していることに注意しましょう。(これをサボろうとすると誤差死します。ていうかしました)
  ...
  \subsection{原始根を用いた畳み込み~\level{4}}
...
\end{verbatim}
  ...
  $(\Z/p\Z)^*$は\defineRuby{巡回群}{じゅんかいぐん}であることが知られています。つまり、ある要素 $g \in (\Z/p\Z)^*$ が存在して、$(\Z/p\Z)^*$ の要素 $x \in (\Z/p\Z)^*$ はすべて、$x = g^i$ の形で表せるということです。このような$g$ のことを、${} \bmod p$ における\defineRuby{原始根}{げんしこん} (\emph{primitive root}) と呼びます。
